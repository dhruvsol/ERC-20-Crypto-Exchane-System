...
contract('EthSwap',([deployer,investor]) => {
    ...
    describe('Buy the Tokens',async () => {
        let result
        before(async () => {
            result = await ethSwap.buy_token({from :investor,value:token_convertor('1')})
        })
    ...
    describe('sell token',async () => {
        ...
        before(async () => {
            ...
            await token.approve(ethSwap.address,token_convertor('100'), {from : investor})
            // then they can sell the token
            result = await ethSwap.sellToken(token_convertor('100'), {from : investor})
        })
        ...
        it('Allow user to sell token on our ETHSwap platform',async () => {
            ...
            assert.equal(ethSwapEtherBalance.toString(),token_convertor('0','Ether'))
            ...
            assert.equal(sellEvent.amount.toString(),token_convertor('100').toString())
            assert.equal(sellEvent.rate.toString(),'100')

             // FAILURE: investor can't sell more tokens than they have
            await ethSwap.sellToken(token_convertor('500'), { from: investor }).should.be.rejected;
